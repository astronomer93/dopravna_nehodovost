...
for(nameDir in nameDirs)
{
	...
	for(traffic in trafficName)
	{
		...
		xlab("1-312, 2-313, 3-314... weeks") + 
		ylab("Correlation") + 
		geom_vline(aes(xintercept=1,linetype="2010-2015"),color="red",show.legend=T) +
		geom_vline(aes(xintercept=53,linetype="2011-2016"),color="orange",show.legend=T) +
		geom_vline(aes(xintercept=105,linetype="2012-2017"),color="yellow",show.legend=T) +
		geom_vline(aes(xintercept=157,linetype="2013-2018"),color="green",show.legend=T) +
		scale_color_manual(values=c("pointColor"="blue")) +						scale_linetype_manual("",values=c("2010-2015"="dashed","2011-2016"="dashed","2012-2017"="dashed","2013-2018"="dashed"),guide=guide_legend(override.aes = list(colour=c("red","orange","yellow","green")))) +
		guides(color=F) +
