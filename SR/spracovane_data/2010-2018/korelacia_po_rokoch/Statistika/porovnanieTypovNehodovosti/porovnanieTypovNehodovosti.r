...
for(nameDir in nameDirs)
{
	...
	for(traffic in trafficName)
	{
		...
		for(rok in 2010:2018)
		{
			...
			nameFilePath <- c('/home/astronomer93/dopravna_nehodovost/SR/spracovane_data/2010-2018/','korelacia_po_rokoch/Data/',nameDir,'/',traffic,'/korelacia/',nameFile)	
			nameFilePath <- paste(nameFilePath,collapse="")
			nameFilePath1 <- c('/home/astronomer93/dopravna_nehodovost/SR/spracovane_data/2010-2018/','korelacia_po_rokoch/Data/',nameDir,'/',traffic)	
			nameFilePath1 <- paste(nameFilePath1,collapse="")	
			file.move(nameFilePath,nameFilePath1)
			setwd('..')

			nameFilePath <- c('/home/astronomer93/dopravna_nehodovost/SR/spracovane_data/2010-2018/','korelacia_po_rokoch/Data/',nameDir,'/',traffic,'/',nameFile)	
			nameFilePath <- paste(nameFilePath,collapse="")	
			nameFilePath1 <- c('/home/astronomer93/dopravna_nehodovost/SR/spracovane_data/2010-2018/','korelacia_po_rokoch/Data/',nameDir,'/')	
			nameFilePath1 <- paste(nameFilePath1,collapse="")	
			file.move(nameFilePath,nameFilePath1)
			setwd('..')
		}	
	...
	{		
		nameFile <- c('porovnanie',rok,cosmicColumnName[cosmicCount],'.csv')
		nameFile <- paste(nameFile,collapse='')
		nameFilePath <- c('/home/astronomer93/dopravna_nehodovost/SR/spracovane_data/2010-2018/','korelacia_po_rokoch/Data/',nameDir,'/',nameFile)	
		nameFilePath <- paste(nameFilePath,collapse="")
